...
//            routeSearch.calculateWalkRouteAsyn(query);
            ...
            List<List<LatLonPoint>> pointsList = new ArrayList<>();
            try {
                scanner = new Scanner(getResources().getAssets().open("avoid_area_data.txt"));
                while (scanner.hasNext()) {
                    String line = scanner.nextLine();
                    List<LatLonPoint> points = new ArrayList<>();
                    String[] data = line.split(";");
                    for (int i = 1; i < data.length; i += 2) {
                        points.add(new LatLonPoint(Double.valueOf(data[i]), Double.valueOf(data[i + 1])));
                    }
                    if (points.size() != 0) {
            ...
            RouteSearch.DriveRouteQuery query = new RouteSearch.DriveRouteQuery(fromAndTo, RouteSearch.DRIVING_MULTI_CHOICE_AVOID_CONGESTION, null, pointsList, null);
